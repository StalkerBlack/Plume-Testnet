...
class Client:
    def __init__(
        ...
        self.w3: AsyncWeb3 = self.get_web3()
    ...
    def get_web3(self) -> AsyncWeb3:
        ...
        Получаем экземпляр класса AsyncWeb3

        """
        if self.proxy:
            # Если прокси переданы, используем их при создании AsyncHTTPProvider
    ...
    async def get_max_priority_fee_per_gas(self, block: dict) -> int:
        ...
        latest_block_transaction_count = await self.w3.eth.get_block_transaction_count(
            block_number
        )
        max_priority_fee_per_gas_lst = []
        for i in range(latest_block_transaction_count):
            try:
                transaction = await self.w3.eth.get_transaction_by_block(
                    block_number, i
                )
    ...
    async def send_transaction(
        ...
        try:
            tx_params["gas"] = int(
                await self.w3.eth.estimate_gas(tx_params) * increase_gas
            )

        except Web3RPCError as error:
            logger.error(
                f"Не удалось выполнить estimate_gas для {self.address} | {error}"
            )
        ...
        return self.w3.to_hex(
            await self.w3.eth.send_raw_transaction(sign.raw_transaction)
        )
